...
// },[subCategory])
   return (
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10 border-t pt-2' >
      {/* Filter options */}
      <div className='min-w-60' >
        <p onClick={()=>setShowFilter(!showFilter)} className='my-2 text-xl flex items-center cursor-pointer gap-2' >FILTERS</p>
        <img className={`h-3 sm:hidden ${showFilter? 'rotate-90': ''}`} src={asset.dropdown} alt=""/>
        {/* Category filter */}
        <div className={`border border-gray-300 pl-5 py-3 mt-6 ${showFilter? '':'hidden'} sm:block `} >
        <p className='mb-3 text-sm font-medium' >CATEGORIES</p>
        <div className='flex flex-col gap-2 text-sm font-light text-gray-700' >
            <p onChange={toggleCategory} className='flex gap-2'>
              <input className='w-3' type="checkbox" value={'men'} name="" id=""  />Men
            </p>
            <p onChange={toggleCategory} className='flex gap-2'>
              <input className='w-3' type="checkbox" value={'women'} name="" id=""  />Women
            </p>
            <p onChange={toggleCategory} className='flex gap-2'>
              <input className='w-3' type="checkbox" value={'kids'} name="" id=""  />Kids
            </p>
        </div>
        </div>
        <div className={`border border-gray-300 pl-5 py-3 mt-6 ${showFilter? '':'hidden'} sm:block `} >
